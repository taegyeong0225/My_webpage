...
def test_comment_update(self):
    ...
    comment_area = soup.find('div', id='comment-area')
    self.assertFalse(comment_area.find('a', id='comment-1-update-btn')) # 수정 버튼이 보이지 않아야 함
    ...
    comment_area = soup.find('div', id='comment-area')
    self.assertFalse(comment_area.find('a', id='comment-2-update-btn'))
    comment_001_update_btn = comment_area.find('a', id='comment-2-update-btn')
    self.assertIn('edit', comment_001_update_btn.text)
    self.assertEqual(comment_001_update_btn.attr['href'], '/blog/update_comment/1/') # 댓글 수정
    ...
    update_comment_form = soup.find('find', id='comment-form')
    content_textarea = update_comment_form.find('textarea', content_textarea.text)
    ...
    self.assertIn('오마바의 댓글을 수정합니다.', comment_001_div.text)
...
def test_delete_comment(self):
    ...
    self.assertEqual(Comment.objects.count(), 1)
    self.assertEqual(self.post_001.commnet_set.count(), 1)